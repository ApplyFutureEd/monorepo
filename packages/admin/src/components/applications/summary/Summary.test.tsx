...
API.post = jest.fn().mockImplementation(() => {
    return { archive: { Key: 'public/paul-cailly-DFEW3.zip' } };
});
Storage.get = jest.fn().mockImplementation(() => {
    return 'https://applyfuture.com/file.zip';
});
window.open = jest.fn();
...
describe('Summary', () => {
    ...
    it('can generate application documents archive', async () => {
        render(
            <Summary
                applicationData={applicationData}
                documentsData={documentsData}
        ...
        const archiveButton = screen.getByText(/zip/);
    ...
    it('can display error toast if application documents archive generation fail', async () => {
        API.post = jest.fn().mockImplementation(() => {
            throw new Error();
        });
